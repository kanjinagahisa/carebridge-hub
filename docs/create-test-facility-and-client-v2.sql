...
-- 
-- 注意: auth.usersテーブルから直接ユーザーIDを取得します
-- ============================================================================
...
-- usersテーブルに存在するユーザーIDを使用（auth.usersではなくusersテーブルから取得）
...
LEFT JOIN auth.users u ON u.id = ufr.user_id
